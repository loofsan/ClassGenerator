...
/*
    This code converts the Json file of Professors.json in the
...
class JsonToJava {
    //Establish File Path for the Json File in the
    // resources folder through Maven
    static String jsonFilePath = "src/main/resources/backend_resources/" +
            "Professors.json";
    ...
    void convertProfJsonToJavaObj(ArrayList<Professor> array) {
        ...
        // Use for-each loop to retrieve each data we
        // need from the Json Node and make it into
        // a professor object, Professor(...)

        for(final JsonNode objNode: profNode) {
            Professor profObject = new Professor(
                    i++, objNode.get("Professor").textValue(),
